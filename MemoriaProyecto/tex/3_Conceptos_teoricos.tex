...
A continuación se describen las métricas que se implementan en este proyecto usando la plantilla de definicion de la norma ISO 9126 \cite{ISO9126}.\\
...
\begin{itemize}
	...
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
	\item \textbf{Interpretación}: $CI \geq 0$, Si se acerca a 1 se definen bien las issues, si alto: no se definen bien las issues, si bajo: desarrollo del proyecto lento
...
\begin{itemize}
	...
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
	\item \textbf{Interpretación}: $MDCI \geq 0$. Cuanto más pequeño mejor.
	\item \textbf{Tipo de escala}: Ratio
	\item \textbf{Tipo de medida}: \textit{NTI, NTIC = Contador}
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
	\item \textbf{Interpretación}: $MDEC \geq 0$. Cuanto más pequeño mejor.
	\item \textbf{Tipo de escala}: Ratio
...
\begin{itemize}
	...
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
	\item \textbf{Interpretación}: $DEPUC \geq 0$. Mejor valores altos
	\item \textbf{Tipo de escala}: Absoluta
...
\begin{itemize}
	...
	\item \textbf{Fórmula}: $RCM = \frac{NTC}{12}$. \textit{RCM = Ratio de cambios por mes, NTC = Número total de commits}
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
...
\begin{itemize}
	...
	\item \textbf{Fuente de medición}: Proyecto en una plataforma de desarrollo colaborativo.
	\item \textbf{Interpretación}: $0 \leq CCP \leq 1$. Mejor valores intermedios
	\item \textbf{Tipo de escala}: Ratio
	\item \textbf{Tipo de medida}: \textit{NCMP, NTC = Contador}
\end{itemize}

\section{Framework de medición} 
Para la implementación de las métricas se ha seguido la solución basada en frameworks propuesta en \textit{Soporte de Métricas con Independencia del Lenguaje para la Inferencia de Refactorizaciones} \cite{marticorena_soporte_2005}. El framework cuyo diseño se muestra en la Figura \ref{fig:MCTMotorMetricas} es independiente del repositorio concreto (\textit{GitLab, GitHub,Bitbucket, SourceForge}) y su objetivo es la reutilización en la implementación del cálculo de métricas.
...
En la Figura  \ref{fig:MCTMotorMetricas} se muestra el diagrama de clases UML que identifica los conceptos principales del framework de medición y la relación entre ellos, especificando la navegabilidad y la multiplicidad. Las anotaciones en forma de flecha especifican los patrones de diseño aplicados en el framework \citep{gamma_patrones_2002}.
